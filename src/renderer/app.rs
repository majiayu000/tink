...
fn render_to_string_impl(element: &Element, width: u16, trim: bool) -> String {
    // Create temporary app components
    let app_helper = RenderHelper;

    app_helper.render_element_to_string_impl(element, width, trim)
}
...
/// Print a message with trimmed trailing spaces (explicit API).
///
/// This is the same as `println()` but makes the trimming behavior explicit.
///
/// # Example
///
/// ```ignore
/// use rnk::prelude::*;
///
/// rnk::println_trimmed("Hello!");
/// rnk::println_trimmed(Text::new("World").into_element());
/// ```
